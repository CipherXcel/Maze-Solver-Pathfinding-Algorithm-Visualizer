...
class MazeSolver extends Component {
    ...
    // Show a toast notification and auto-dismiss after 3 s.
    ...
    animateShortestPath = (nodesOptPathOrdered, nodesVisitedOrdered) => {
        ...
        // No path found — just one node (the start).
        if (nodesOptPathOrdered.length === 1) {
            this.setState({
                visualizingAlgorithm: false,
                visitedCount: nodesVisitedOrdered ? nodesVisitedOrdered.length : 0,
                pathLength: 0,
            });
        ...
        for (let i = 1; i < nodesOptPathOrdered.length; i++) {
            ...
            if (i === nodesOptPathOrdered.length - 1) {
                ...
                setTimeout(() => {
                    ...
                    const visited = nodesVisitedOrdered ? nodesVisitedOrdered.length : 0;
                    const pathLen = nodesOptPathOrdered.length;
                    ...
                    this.showToast(`Path found — ${pathLen} steps! ✓`, "success");
        ...
        for (let i = 1; i <= nodesVisitedOrdered.length; i++) {
            ...
            if (i === nodesVisitedOrdered.length) {
                ...
                setTimeout(() => {
                    this.setState({
                        visualizingAlgorithm: false,
                        visitedCount: nodesVisitedOrdered.length,
                        pathLength: 1,
                    });
                    this.showToast("Goal reached! ✓", "success");
                }, i * this.state.speed);
        ...
        for (let i = 0; i <= walls.length; i++) {
            ...
            if (i === walls.length) {
                ...
                setTimeout(() => {
                    ...
                    this.showToast(`Maze generated — ${walls.length} walls ⬛`, "info");
    ...
    render() {
        ...
        return (
            ...
            <React.Fragment>
                ...
                {/* Toast notification */}
                {this.state.toast && (
                    <div className={`ms-toast ${this.state.toast.type}`} role="status" aria-live="polite">
                        <span className="toast-icon">
                            {this.state.toast.type === "success" ? "✓" :
                             this.state.toast.type === "error"   ? "✗" : "ℹ"}
                        </span>
                        <span className="toast-text">{this.state.toast.message}</span>
                    </div>
                )}
